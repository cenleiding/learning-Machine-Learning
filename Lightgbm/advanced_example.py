...
@description:
free_raw_data 的使用
feature_name ， categorical_feature的使用
save_model 的使用 .Booster加载模型
init_model 实现增量训练
learning_rates 动态训练速度
callbacks 训练中修改参数
fobj ：自定义目标函数
feval ：自定义评估函数
"""
...
# 默认free_raw_data=True,释放原数据节约内存
...
# feature_name 如果用了pandas则默认pandas的columns name
# categorical_feature 可以是字符串列表，也可以是int列表。如果用了pandas则默认categorical columns。
